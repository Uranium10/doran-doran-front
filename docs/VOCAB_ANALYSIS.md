...
- 분석 창 맨 위에 저장된 원전 제목이 있으면 **참고한 옛 이야기**를 보여준다. 일반/옛이야기 모드는 `generation.source_title`, 그림 동화는 `generation.reference_source_title`을 쓴다. 미기록·검색 실패·옛 그림 동화의 `내 그림 이야기`는 표시하지 않는다.
...
원전 제목은 이 경량 분석 API의 결과가 아니라, 책을 열 때 받은 저장 동화 `generation` 메타데이터에서 읽는다. 검색용 각색본 제목 대신 자료에 별도 보관한 실제 원전명을 사용하며, 기존 동화도 저장된 원전 ID로 조회 응답의 제목만 보정한다. 원전 ID가 없으면 임의로 연결하지 않는다. 새 그림 동화에는 검색한 참고 원전의 ID·제목을 별도 저장한다. 이 기록은 계획에 전달된 참고 자료이며, 최종 문장에 실제로 차용됐다는 판정은 아니다.
