...
| 조건 | 지급 |
|---|---|
| 문항 수 5개 이상인 독후 퀴즈에서 5개 이상 정답 | 이야기 친구(friend) |
| 위 조건을 충족하고 전체 정답 | 완벽한 한 권(mastery) 추가 |
| 첫 시도에서 만점 | 두 보상 모두 |
| 다시 풀어서 조건 충족 | 아직 받지 않은 보상만 |
| 같은 답안 재전송 / 같은 책 반복 만점 | 중복 지급 없음 |
| 배치고사·체크리스트 / 5문항 미만 | 스티커 지급 없음 |
...
마이그레이션: `backend_test/sql/20260918_sticker_book.sql`.
