...
import {PenTool,Eraser,LassoSelect,Undo2,Redo2,Trash2,PaintBucket,SprayCan,ArrowLeftRight,Pipette,Palette,RotateCcw,Pointer,Sticker,Wallpaper,X,MoreHorizontal} from 'lucide-react'
...
import {DEFAULT_LAYERS,UNDO_LIMIT,type LayerSettings,type SketchDocument} from '@/lib/sketchbook-document'
...
export function Sketchbook({value,onChange,layers=DEFAULT_LAYERS,storageKey,onDocumentChange,background='#fffaf0',onBackgroundChange,onBusyChange,disabled=false,width:paperWidth=SKETCH_WIDTH,height:paperHeight=SKETCH_HEIGHT}:{value:Stroke[];layers?:LayerSettings;storageKey?:string;onDocumentChange?:(doc:SketchDocument)=>void;onChange:(v:Stroke[])=>void;background?:string;onBackgroundChange?:(color:string)=>void;onBusyChange?:(busy:boolean)=>void;disabled?:boolean;width?:number;height?:number}){
  ...
  const [history,setHistory]=useState<SketchDocument[]>([]),[future,setFuture]=useState<SketchDocument[]>([]),[clear,setClear]=useState(false),[notice,setNotice]=useState(''),[picker,setPicker]=useState(false),[custom,setCustom]=useState<string[]>([])
  ...
  const remember=(doc:SketchDocument)=>{setHistory(h=>[...h.slice(-(UNDO_LIMIT-1)),doc]);setFuture([])}
  ...
  const commit=(next:Stroke[])=>{remember(documentRef.current);applyDocument({...documentRef.current,strokes:next})}
  const changeBackground=(color:string)=>{remember(documentRef.current);applyDocument({...documentRef.current,background:color})}
  ...
  const layerEnd=(cancel:boolean)=>{const before=layerStart.current;layerStart.current=null;if(!before)return;if(cancel)applyDocument(before);else if(before.layers!==documentRef.current.layers)remember(before)}
  ...
  useEffect(()=>{if(canvas.current)paintSketch(canvas.current,value,layers)},[value,W,H,layers])
  ...
  const undo=()=>{if(blocked||active.current||!history.length)return;setFuture(f=>[...f,documentRef.current]);applyDocument(history.at(-1)!);setHistory(h=>h.slice(0,-1));setSelected([])}
  const redo=()=>{if(blocked||active.current||!future.length)return;setHistory(h=>[...h.slice(-(UNDO_LIMIT-1)),documentRef.current]);applyDocument(future.at(-1)!);setFuture(f=>f.slice(0,-1));setSelected([])}
  ...
  return <div className={styles.studio} onKeyDown={e=>{
    ...
    <aside className={styles.rail} aria-label="그리기 도구">
      ...
      {storageKey&&<button type="button" aria-label="그림 저장 및 불러오기" title="저장 · 불러오기" disabled={blocked} onClick={()=>{if(active.current)return;setStorage(true);setPicker(false);setExtras(null)}}><MoreHorizontal size={24}/></button>}
    </aside>
    <div className={styles.stage}>
      ...
      <div className={styles.layerDock} aria-label="그림 레이어" data-expanded={layerLabel}>{(['outline','color'] as const).map(layer=><LayerControl key={layer} label={layer==='outline'?'밑그림':'색칠'} active={activeLayer===layer} value={layers[layer]} disabled={blocked} onSelect={()=>selectLayer(layer)} onBegin={()=>{if(active.current||layerStart.current||fillBusy.current)return false;layerStart.current=documentRef.current;return true}} onChange={setting=>layerChange(layer,setting)} onEnd={layerEnd}><LayerGlyph kind={layer}/></LayerControl>)}</div>
      ...
      <div className={styles.status} role="status">{filling?'색을 채우고 있어요…':notice||(tool==='lasso'?(selected.length?'가운데는 이동 · 테두리는 크기 · 위쪽은 회전':'선을 둘러서 그림을 골라요'):tool==='pick'?'고를 색과 지금 색을 함께 확인해요':tool==='fill'?`허용 오차 ${tolerance} · 높일수록 더 넓은 색 범위를 채워요`:'두 손가락으로 확대·이동·회전해요')}{tool==='lasso'&&selected.length>0&&<button type="button" disabled={blocked} aria-label="선택한 그림 삭제" onClick={()=>{commit(value.filter(s=>!selected.includes(s.id)));setSelected([])}}><Trash2 size={20}/></button>}</div>
    </div>
    ...
    {storageKey&&<SketchStorage scope={storageKey} open={storage} onClose={()=>setStorage(false)} current={documentRef.current} enabled={!disabled} isBusy={()=>Boolean(active.current||fillBusy.current||layerStart.current)} onLoad={doc=>{remember(documentRef.current);applyDocument(doc);changeView({...IDENTITY});setStorage(false);announce('그림을 불러왔어요')}} onNotice={setNotice}/>}
