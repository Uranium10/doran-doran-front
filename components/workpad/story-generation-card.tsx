...
export function StoryGenerationCard({ bookshelfMode = false, latestUnread, finishing = false }: { bookshelfMode?: boolean; latestUnread?: BookSummary | null; finishing?: boolean } = {}) {
  ...
  const { job, error, starting, canRetry, retry, connectionLost, dismiss, openStory } = useGeneration()
  const { profiles, selectProfile } = useProfile()
  const reopen = () => {
    if (job?.profile_id && profiles.some(profile => profile.id === job.profile_id)) selectProfile(job.profile_id)
    // 폼을 열 때는 실패 안내를 먼저 지우지 않는다. 입력을 바꿔 제출하거나 나중에 닫을 수 있다.
  ...
  const helper = failed && !canRetry ? "이야기 재료를 확인하고 다시 시작해 주세요." : connectionLost ? "책방 소식을 다시 확인하고 있어요. 잠시만 기다려 주세요." : done ? "아직 펼치지 않은 새 이야기가 기다려요." : finishing ? "책장 정보가 도착하면 바로 펼칠 수 있어요." : failed ? "" : job?.mode === "legacy" ? "다른 메뉴를 둘러봐도 괜찮아요. 이 페이지를 새로고침하지는 말아 주세요." : "다른 곳을 둘러봐도 동화는 계속 만들어져요. 완성되면 알려 드릴게요."
  return <div className={styles.card} aria-busy={active || finishing || starting} data-failed={failed} data-generation-card data-testid={done && bookshelfMode ? "unread-story-card" : "generation-card"}>
    <PaperTheatre stopped={done || failed || finishing} failed={failed}/>
