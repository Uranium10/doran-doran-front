...
const MODES = [{ id: "original", label: "옛이야기", icon: BookOpen }, { id: "personalized", label: "내 이야기", icon: Sparkles }] as const
type Draft = { mode: StoryMode; themeId: string; favorite: string; protagonistName: string; eventText: string }
function isDraft(value: unknown): value is Draft {
  return objectValue(value) && (value.mode === "original" || value.mode === "personalized")
    && ["themeId", "favorite", "protagonistName", "eventText"].every(key => typeof value[key] === "string")
}

export function StorySetup({ profileId, defaultName, persistenceKey, onSubmit }: {
  profileId: string; defaultName: string; persistenceKey?: string | null; onSubmit: (input: StoryInput) => void
}) {
  ...
  const { mode, themeId, favorite, protagonistName, eventText } = draft.value
  ...
  useEffect(() => {
    ...
    void fetchStoryThemes(profileId, mode, controller.signal).then(result => {
      ...
      const next = drawThemeCards(result.topics)
  ...
  const changeMode = (next: StoryMode) => {
    if (next !== mode) { setLoadedMode(null); update({ mode: next, themeId: "" }) }
  }
  const reroll = () => { setCards(drawThemeCards(catalog, cards)); update({ themeId: "" }) }
  ...
  const canSubmit = draft.ready && loadedMode === mode && Boolean(preferences) && !saving
    && cards.some(t => t.theme_id === themeId) && (mode === "original" || protagonistName.trim().length > 0)
  const handleSubmit = () => {
    if (!canSubmit) return
    onSubmit({ mode, themeId, protagonistName: mode === "original" ? "" : protagonistName.trim(),
  ...
  return <div className="mx-auto w-full max-w-2xl">
    ...
    <div className="overflow-hidden rounded-3xl border border-border bg-card shadow-md">
      ...
      <section role="tabpanel" id="story-mode-panel" aria-labelledby={`story-tab-${mode}`} className="space-y-7 p-5 sm:p-8">
        <p className="min-h-12 text-center text-sm leading-relaxed text-muted-foreground">
        ...
        <fieldset className="min-w-0 space-y-4">
          <legend className="w-full font-heading text-xl text-foreground">오늘은 이런 동화를 준비했어요</legend>
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-sm text-muted-foreground">마음에 드는 이야기를 골라 주세요</p>
          ...
          {themeError ? <div role="alert" className="rounded-2xl bg-secondary/50 p-5 text-sm">{themeError}
            ...
            : cards.length === 0 ? <p role="status">아직 준비된 주제가 없어요. 내 이야기 탭에서 만나보세요.</p>
            : <div className="grid gap-3 sm:grid-cols-3">{cards.map((topic, index) => <label key={topic.theme_id}
                ...
                <input type="radio" name="story-theme" value={topic.theme_id} checked={themeId === topic.theme_id}
                  onChange={() => update({ themeId: topic.theme_id })} className="peer sr-only" />
        ...
        {mode === "personalized" && <div className="space-y-5 border-t border-border pt-6">
          ...
          <div className="space-y-2"><Label htmlFor="today-event">오늘 있었던 일 <span className="text-muted-foreground">(선택)</span></Label>
        ...
        <Button onClick={handleSubmit} disabled={!canSubmit} size="lg" className="h-13 w-full rounded-full text-base">
          <Wand2 className="mr-1 h-5 w-5" aria-hidden="true" />{mode === "original" ? "옛이야기 만나기" : "내 이야기 만들기"}
        </Button>
      </section>
    </div>
  </div>
