...
export function SketchStorage({scope,current,enabled,isBusy,open,onClose,onLoad,onNotice}:{scope:string;current:SketchDocument;enabled:boolean;isBusy:()=>boolean;open:boolean;onClose:()=>void;onLoad:(doc:SketchDocument)=>void;onNotice:(s:string)=>void}){
  ...
  useEffect(()=>{
    if(!open)return
    let cancelled=false;setConfirm(null);setMessage('저장한 그림을 확인하고 있어요.');setBusy(true)
  ...
  return <dialog ref={dialog} className={styles.storageDialog} aria-labelledby="sketch-storage-title" onCancel={e=>{e.preventDefault();if(!busy)onClose()}}>
    <header><div><h3 id="sketch-storage-title">내 그림 보관함</h3><p>이 기기·브라우저에만 저장돼요.</p></div><button type="button" aria-label="보관함 닫기" disabled={busy} onClick={onClose}><X size={21}/></button></header>
    ...
    {confirm?<section className={styles.storageConfirm}><strong>{confirm==='save'?'이 칸의 그림을 바꿀까요?':'저장한 그림을 불러올까요?'}</strong><p>{confirm==='save'?'전에 저장한 그림을 덮어써요.':'현재 그림이 바뀌어요. 실행 취소로 되돌릴 수 있어요.'}</p><div><button type="button" disabled={busy} onClick={()=>setConfirm(null)}>취소</button><button type="button" disabled={busy} onClick={()=>void(confirm==='save'?save():load())}>{confirm==='save'?'덮어쓰기':'불러오기'}</button></div></section>:<footer><button type="button" disabled={busy||choice==='auto'} onClick={()=>slots[choice]?setConfirm('save'):void save()}><Save size={18}/>저장</button><button type="button" disabled={busy||!slots[choice]} onClick={()=>setConfirm('load')}><FolderOpen size={18}/>불러오기</button></footer>}
  </dialog>
}
