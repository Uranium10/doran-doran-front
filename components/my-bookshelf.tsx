...
export function MyBookshelf({ onCreate }: { onCreate: () => void }) {
  ...
  useEffect(() => {
    ...
    let active = true
    setState({ profileId })
    fetchBookshelf(profileId,0,7).then(data => { if(active) setState({ profileId,data }) })
      .catch(() => { if(active) setState({ profileId,error:"책장 소식을 가져오지 못했어요." }) })
    return () => { active=false }
  },[profileId,generation.job?.status,retry])
  ...
  return <div>
    ...
    {(generation.job || generation.error) && <div className="mb-8"><StoryGenerationCard/></div>}
    ...
    <div className={styles.shelf}>
      ...
      {data?.stories.map(book=><div className={styles.slot} key={book.story_id}><Link href={bookPath(book.story_id,"dashboard")} aria-label={`${book.title} 읽기`}><BookCover book={book}/></Link><p className={styles.bookLabel}>{book.reading_progress?.completed_at ? "다 읽은 이야기" : book.reading_progress ? "읽고 있는 이야기" : "새로운 이야기"}</p></div>)}
