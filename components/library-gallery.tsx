...
export function LibraryGallery(){
  ...
  return <div><div className="mb-8 flex items-end justify-between gap-3"><div><p className="text-xs tracking-widest text-primary">우리 아이가 만든 이야기</p><h1 className="mt-2 font-heading text-3xl">전체 책장</h1><p className="mt-2 text-sm text-muted-foreground">{data?`${data.total}권의 모험이 기다리고 있어요.`:'한 권씩 쌓이는 나만의 이야기'}</p></div>
    ...
    :<><div className={list?'space-y-3':'grid grid-cols-2 gap-x-6 gap-y-9 sm:grid-cols-3 lg:grid-cols-4'}>{data.stories.map(book=><article key={book.story_id} className={list?styles.row:'min-w-0'} style={list?{'--book-color':normalizeCoverColor(book.cover_color)} as CSSProperties:undefined}>
      <Link href={bookPath(book.story_id,'library')} className="block" aria-label={`${book.title} 읽기`}><>{list?<LibraryThumbnail book={book}/>:<BookCover book={book}/>}</></Link>
    ...
    {history && history.profileId===profileId && <BookQuizHistory key={`${profileId}:${history.book.story_id}`} profileId={profileId} book={history.book} onClose={()=>setHistory(null)}/>}
