...
import { LayoutGrid,List,Trash2,BookOpen,ClipboardList } from "lucide-react"
...
export function LibraryGallery({from="library"}:{from?:BookOrigin}){
  ...
  const [history,setHistory]=useState<{profileId:string;book:BookSummary}|null>(null)
  ...
  return <div><div className="mb-8 flex items-end justify-between gap-3"><div><p className="text-xs tracking-widest text-primary">우리 아이가 만든 이야기</p><h1 className="mt-2 font-heading text-3xl">전체 책장</h1><p className="mt-2 text-sm text-muted-foreground">{data?`${data.total}권의 모험이 기다리고 있어요.`:'한 권씩 쌓이는 나만의 이야기'}</p></div>
    ...
    {history && history.profileId===profileId && <BookQuizHistory from={from} key={`${profileId}:${history.book.story_id}`} profileId={profileId} book={history.book} onClose={()=>setHistory(null)}/>}
