...
import { bookPath, returnPath, openStoryAssessment } from "@/lib/bookshelf"
...
export function BookReaderPage({storyId,from,view}:{storyId:string;from:string;view:string}) {
  ...
  const origin=from==='library'?'library':'dashboard'
  ...
  useEffect(()=>{
    ...
    openStoryAssessment(profileId,storyId).then(attempt=>{
      if(!active)return
  ...
  },[view,profileId,storyId,screen.ready,Boolean(assessment),retry,origin,router,screen.setValue])
  ...
  return <div className="min-h-screen bg-background"><AppHeader/>
    ...
    :<RemoteBook key={`${currentProfile.id}:${storyId}`} profileId={currentProfile.id} storyId={storyId} persistenceKey={`${sessionScope}:book:${storyId}`} pages={assessment.pages} title={assessment.title} coverImage={assessment.cover_image} coverColor={assessment.cover_color} childName={currentProfile.name} hasQuiz={assessment.quizzes.length>0}
      onExit={()=>router.push(returnPath(origin))} exitLabel={origin==='library'?'전체 책장으로 돌아가기':'내 책장으로 돌아가기'} onFinish={()=>router.push(assessment.quizzes.length?`${bookPath(storyId,origin)}&view=quiz`:returnPath(origin))}/>}
  </div>
}
