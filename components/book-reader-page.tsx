...
import { bookPath, bookOrigin, returnLabel, returnPath, openStoryAssessment, fetchStoryAttempt } from "@/lib/bookshelf"
...
export function BookReaderPage({storyId,from,view}:{storyId:string;from:string;view:string}) {
  ...
  return <div className="min-h-screen bg-background"><AppHeader/>
    {error?<main className="mx-auto max-w-xl px-5 py-16"><BackLink href={returnPath(origin)} label={returnLabel(origin)}/><p role="alert" className="my-6">{error}</p><button onClick={()=>{setError(null);setRetry(v=>v+1)}} className="rounded-full bg-primary px-6 py-3 text-primary-foreground">다시 불러오기</button></main>
    ...
    :<RemoteBook key={`${currentProfile.id}:${storyId}`} profileId={currentProfile.id} storyId={storyId} vocabulary={{ profileId: currentProfile.id, storyId, initialAnalysis: assessment.generation?.vocab_analysis, sourceMode: assessment.generation?.mode, sourceTitle: assessment.generation?.source_title, referenceSourceTitle: assessment.generation?.reference_source_title }} persistenceKey={`${sessionScope}:book:${storyId}`} pages={assessment.pages} readingLevel={assessment.generation?.level} title={assessment.title} coverImage={assessment.cover_image} coverColor={assessment.cover_color} childName={currentProfile.name} hasQuiz={assessment.quizzes.length>0}
      pdfExport={{path:`/exports/stories/${encodeURIComponent(storyId)}.pdf?profile_id=${encodeURIComponent(currentProfile.id)}`,filename:`${assessment.title||"도란도란 이야기"}-동화책.pdf`}}
      onExit={()=>router.push(returnPath(origin))} exitLabel={returnLabel(origin)} onFinish={()=>router.push(assessment.quizzes.length?`${bookPath(storyId,origin)}&view=quiz`:returnPath(origin))}/>}
  </div>
}
